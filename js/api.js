...
(function (global) {
  ...
  var API_BASE = "https://oneere2-api.onrender.com";
  ...
  /* Distinguer les pannes, parce qu'elles n'ont pas la même solution :
       - "ok"          : le serveur répond, tout va bien ;
       - "absent"      : quelque chose répond à cette adresse, mais pas notre
                         API (service Render pas créé, adresse erronée, ou
                         ALLOWED_ORIGINS mal réglé) ;
       - "injoignable" : rien du tout ne répond (pas de réseau, nom de
                         domaine inexistant).

     Le piège, et c'est lui qui m'a fait écrire un mauvais message la
     première fois : quand Render renvoie sa page 404 « service inconnu »,
     cette page ne porte pas l'en-tête CORS. Le navigateur refuse alors de
     nous la montrer, et fetch échoue EXACTEMENT comme s'il n'y avait pas
     de réseau. Impossible de faire la différence à ce stade.

     D'où la seconde tentative en mode "no-cors" : elle ne permet pas de
     lire la réponse, mais elle aboutit si — et seulement si — quelqu'un a
     répondu au bout du fil. C'est tout ce qu'on a besoin de savoir. */
  function reveiller() {
    return fetch(API_BASE + "/api/health", { method: "GET", mode: "cors" })
      ...
      .catch(function () {
        return fetch(API_BASE + "/api/health", { mode: "no-cors", cache: "no-store" })
          .then(function () { return { etat: "absent", code: "réponse illisible" }; })
