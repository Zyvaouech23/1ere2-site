  ...
  function verifierServeur() {
    ...
    API.reveiller().then(function (resultat) {
      if (resultat.etat === "absent") {
        // Quelque chose répond, mais pas notre API. Attendre n'y changera
        // rien : le message doit dire quoi faire, pas « patiente ».
        etatServeur("panne", "L'adresse " + API.base + " répond, mais ce n'est pas "
          + "l'API de dépôt (" + resultat.code + "). Inutile d'attendre : soit le service "
          + "n'est pas encore créé sur Render, soit l'adresse de js/api.js ne correspond "
          + "pas au nom du service, soit ALLOWED_ORIGINS n'autorise pas ce site.");
        el.bouton.disabled = true;
        return;
      }
      if (resultat.etat === "injoignable") {
        etatServeur("panne", "Aucune réponse de " + API.base + " : ni le serveur, "
          + "ni même l'adresse. Vérifie ta connexion, puis l'adresse dans js/api.js.");
        el.bouton.disabled = true;
        return;
      }
